  ...
  return <div className="background-page">
    ...
    <main>
      <section className="background-hero" aria-labelledby="background-title">
        <div className="bg-container hero-grid"><div><img className="portrait" src="/images/mario-sorgente.webp" alt="Mario Sorgente" width={400} height={400} /><p className="eyebrow">Senior Product Manager and AI Product Builder</p><h1 id="background-title">Mario Sorgente</h1><h2>I build AI, platform and SaaS products.</h2></div><div className="hero-copy"><p>Senior Product Manager with 7+ years of experience building B2B and B2C products across AI systems, LLM-powered workflows, agent design, SaaS platforms, energy and deep-tech.</p><p>I turn complex technical and commercial problems into clear product decisions.</p><div className="hero-actions"><a className="primary-action" href="#experience">View experience <ArrowDownRight size={17}/></a><a className="text-action" href={linkedIn}>LinkedIn profile <ArrowUpRight size={16}/></a></div><address><span><MapPin size={15}/>Working remotely</span></address></div></div>
        <div className="bg-container status-line"><span /><p>Product leadership, AI products and platform strategy</p></div>
      </section>
      ...
      <section id="profile" className="bg-section"><div className="bg-container"><SectionHeading label="Profile" title="How I work across technology, users and business."/><div className="profile-layout"><div className="profile-copy"><p>Mario is a Senior Product Manager and product builder with experience across AI, SaaS, platform products, energy systems, deep-tech hardware and B2C digital products.</p><p>He turns unclear opportunities into product direction, leads cross-functional teams and takes products from discovery through launch.</p><p>His work spans public APIs, LLM workflows, AI agents, platform architecture, product packaging, onboarding, access control and hardware-software systems.</p><p>He combines strategy, technical understanding and commercial awareness to build products people can use and a business can sustain.</p></div><div className="pillars">{[['Product Strategy','Vision, discovery, prioritisation, roadmaps, business models and go-to-market.'],['AI Product Development','LLM workflows, AI agents, NLP, evaluation, safeguards and human review.'],['Platform & SaaS','Public APIs, RBAC, multitenancy, integrations, onboarding and scalable architecture.']].map(([title,copy],i)=><article key={title}><span>0{i+1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></div></div></section>

      <section id="experience" className="bg-section experience-section"><div className="bg-container"><SectionHeading label="Experience" title="Four chapters of product work."/>{chapters.map(chapter => <div className="chapter" id={chapter.id} key={chapter.id}><aside><span>Chapter {chapter.number}</span><h3>{chapter.title}</h3><p>{chapter.introduction}</p></aside><div>{chapter.companies.map(company => <CompanyBlock company={company} key={company.name}/>)}</div></div>)}</div></section>

      <section id="expertise" className="bg-section expertise-section"><div className="bg-container"><SectionHeading label="Core expertise" title="What I know well."/><div className="expertise-grid">{Object.entries(expertise).map(([group,items],i)=><article key={group}><span>0{i+1}</span><h3>{group}</h3><ul>{items.map(item=><li key={item}>{item}</li>)}</ul></article>)}</div></div></section>

      <section id="education" className="bg-section credentials-section"><div className="bg-container"><SectionHeading label="Education & credentials" title="Where I studied, and what I keep learning."/><div className="credentials-grid"><div className="education-list">{education.map(([school,degree,field,dates])=><article key={`${school}-${degree}`}><div><h3>{school}</h3><p>{degree}</p><span>{field}</span></div><time>{dates}</time></article>)}</div><aside><h3>Languages</h3>{languages.map(([language,level])=><p key={language}><strong>{language}</strong><span>{level}</span></p>)}</aside></div></div></section>
